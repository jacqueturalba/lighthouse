...
final class PromotionKitRequestService
{
    public function request(int $kitId, int $userId): string
    {
        ...
        if(PromotionKitRequest::forUserAndKit($kitId,$userId)) return 'You have already requested this promotion kit.';
        try { PromotionKitRequest::create($kitId,$userId); log_event('promotion_kit_requested',['kit_id'=>$kitId,'user_id'=>$userId]); return 'Download request submitted.'; }
