...
(() => {
  ...
  document.addEventListener('submit',async e=>{const form=e.target.closest('[data-modal-edit-form]');if(!form)return;e.preventDefault();const button=form.querySelector('[type="submit"]'),body=form.closest('[data-comment-body]'),fd=new FormData(form);fd.append('_token',token);button.disabled=true;try{const r=await fetch('/sflex/comments/'+form.closest('[data-comment-id]').dataset.commentId+'/edit',{method:'POST',body:fd}),d=await r.json();if(!r.ok)throw new Error(d.error||'Could not edit comment.');body.textContent=d.body;}catch(error){alert(error.message);}finally{button.disabled=false;}},true);
