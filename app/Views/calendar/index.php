...
<div class="row g-4">
  <div class="col-xl-8">
    <section class="lh-card p-3 p-md-4">
      ...
      <div class="lh-calendar-grid lh-calendar-weekdays mb-2">
      ...
      ?>
        <div class="lh-calendar-day <?= $inMonth ? '' : 'is-muted' ?><?= $dateKey === $today ? 'is-today' : '' ?>">
          <div class="lh-calendar-number">
            <?= $day->format('j') ?>
          </div>
          <?php foreach (array_slice($eventsByDate[$dateKey] ?? [], 0, 3) as $event): ?>
            <a class="lh-calendar-event" href="/events/<?= (int)$event['id'] ?>" title="<?= e($event['title']) ?>">
              <?= e($event['title']) ?>
            </a>
          <?php endforeach; ?>
        </div>
        ...
        <div class="d-grid gap-2">
          <?php foreach ($pending as $event): ?>
            <a class="lh-card p-3 text-decoration-none" href="/events/<?= (int)$event['id'] ?>">
              <div class="small text-warning-emphasis fw-semibold">Pending review · <?= e(date('M j, Y', strtotime($event['event_date']))) ?></div>
              <div class="fw-semibold text-dark mt-1"><?= e($event['title']) ?></div>
              <div class="small text-secondary mt-1"><?= e($event['location']) ?></div>
            </a>
          <?php endforeach; ?>
        </div>
      </section>
