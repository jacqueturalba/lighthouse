...
public static function commentPostId(int $id):?int{$q=db()->prepare('SELECT post_id FROM sflex_comments WHERE id=?');$q->execute([$id]);$post=$q->fetchColumn();return$post===false?null:(int)$post;}
...
public static function find(int $id,int $u,bool $admin=false):?array{$q=db()->prepare(self::select().' WHERE p.id=?');$q->execute([$u,$id]);$p=$q->fetch();if(!$p)return null;$posts=[$p];self::hydrate($posts);$p=$posts[0];$p['comments']=self::comments($id,$admin);return$p;}
}
