...
public static function comment(int $p,int $u,string $b):void{db()->prepare('INSERT INTO sflex_comments(post_id,user_id,body) VALUES(?,?,?)')->execute([$p,$u,$b]);}
public static function reply(int $p,int $u,int $parent,string $b):bool{$q=db()->prepare('SELECT id FROM sflex_comments WHERE id=? AND post_id=? AND parent_id IS NULL');$q->execute([$parent,$p]);if(!$q->fetchColumn())return false;db()->prepare('INSERT INTO sflex_comments(post_id,parent_id,user_id,body) VALUES(?,?,?,?)')->execute([$p,$parent,$u,$b]);return true;}
