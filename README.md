...
4. Ensure the web-service user can write `storage/logs` and `storage/sessions`, `chmod -R +w storage/` for local, `chmod -R g+rwX storage/` for production then serve the `public` folder: `php -S localhost:8000 -t public`.
